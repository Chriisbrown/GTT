...
event_num = 1000
name = "performance_plots/finalGTT/"
emulation_df = emulation(event_num,"/home/cb719/Documents/L1Trigger/GTT/EMP/fw-work/src/GTT/DataFiles/TT_object_300k.root")
print(".........Emulated...........")
simulation_df = simulation(event_num,"/home/cb719/Documents/L1Trigger/GTT/EMP/fw-work/src/GTT/DataFiles/TT_object_300k.root")
print("........Simulated...........")
fw_sim_df = fw_sim_reader(event_num,"/home/cb719/Documents/L1Trigger/GTT/EMP/fw-work/src/GTT/TestBench/output_files/sim/FullGTT")
print(".....FPGA Sim file read.........")
fw_hw_df = fw_hw_reader(event_num,"/home/cb719/Documents/L1Trigger/GTT/EMP/fw-work/src/GTT/TestBench/output_files/fpga")
...
MET_df_old.insert(0,"fpga_MET",fw_hw_df["fpga_MET"],True)

vtx_df = fw_sim_df[["fw_z0","fw_z0_weight"]].join(fw_hw_df[["fpga_z0","fpga_z0_weight"]].join(emulation_df[["EM_Vertex","EM_Vtx_Weight"]].join(simulation_df[["SW_Vertex","SW_Vtx_Weight","Pv_z0","Pv_weight","MCVertex"]])))

#for i in range(len(MET_df_old)):
  #print(MET_df_old[["EM_MET","fw_MET","fpga_MET"]].iloc[i])
  #print(vtx_df.iloc[i])
...
vtx_df["VTX_EM_error"] = abs(vtx_df["fw_z0"] - vtx_df["EM_Vertex"])

MET_df["MET_SW_error"] = abs(MET_df["fw_MET"] - MET_df["SW_MET"])
vtx_df["VTX_SW_error"] = abs(vtx_df["fw_z0"] - vtx_df["SW_Vertex"])

MET_df["MET_fpga_error"] = abs(MET_df["fw_MET"] - MET_df["fpga_MET"])
vtx_df["VTX_fpga_error"] = abs(vtx_df["fw_z0"] - vtx_df["fpga_z0"])

print(MET_df.nlargest(10,'MET_fpga_error'))
print(vtx_df.nlargest(10,'VTX_fpga_error'))
...
ax[1].grid()
plt.tight_layout()
plt.savefig(name+"vertexemvsfpgasim.png")
...
ax[1].grid()
plt.tight_layout()
plt.savefig(name+"vertexsimvsfpgasim.png")

#=========================================FPGA Vertex========================================================#
fig,ax = plt.subplots(1,2,figsize=(18,9))
slope, intercept, r_value, p_value, std_err = stats.linregress(vtx_df["fpga_z0"],vtx_df["fw_z0"])

ax[0].tick_params(axis='x', labelsize=16)
ax[0].tick_params(axis='y', labelsize=16)    
ax[0].set_xlabel("PV$_{fpga}$ [cm]",loc="right",fontsize=16)
ax[0].set_ylabel("PV$_{fw}$ Vertex [cm]",loc="top",fontsize=16)
ax[0].scatter(vtx_df["fpga_z0"],vtx_df["fw_z0"])
ax[0].plot(vtx_df["fpga_z0"], intercept + slope*vtx_df["fpga_z0"], 'r', label="slope: %f    intercept: %f" % (slope, intercept))
ax[0].set_xlim(-15,15)
ax[0].set_ylim(-15,15)
ax[0].legend(fontsize=16)
ax[0].grid()
...
ax[1].tick_params(axis='x', labelsize=16)
ax[1].tick_params(axis='y', labelsize=16)    
ax[1].set_xlabel("PV$_{fpga}$ - PV$_{fw}$ [cm]",loc="right",fontsize=16)
ax[1].set_ylabel("# Events",loc="top",fontsize=16)
ax[1].hist((vtx_df["fpga_z0"]-vtx_df["fw_z0"]),histtype="step",bins=17,range=(-1,1))
ax[1].grid()
plt.tight_layout()
plt.savefig(name+"vertexfpgasimvsfpga.png")
...
ax[1].grid()
plt.tight_layout()
plt.savefig(name+"metemvsfpgasim.png")
...
ax[1].grid()
plt.tight_layout()
plt.savefig(name+"metsimvsfpgasim.png")

#=======================================FPGA MET==========================================================#

fig,ax = plt.subplots(1,2,figsize=(18,9))
slope, intercept, r_value, p_value, std_err = stats.linregress(MET_df["fpga_MET"],MET_df["fw_MET"])

ax[0].tick_params(axis='x', labelsize=16)
ax[0].tick_params(axis='y', labelsize=16)    
ax[0].set_xlabel("$E^T_{miss,fpga}$ [GeV]",loc="right",fontsize=16)
ax[0].set_ylabel("$E^T_{miss,fw}$ [GeV]",loc="top",fontsize=16)
#ax[0].set_xlim(0,350)
#ax[0].set_ylim(0,350)
#ax[0].set_xscale("log")
#ax[0].set_yscale("log")
ax[0].scatter(MET_df["fpga_MET"],MET_df["fw_MET"])
ax[0].plot(MET_df["fpga_MET"], intercept + slope*MET_df["fpga_MET"], 'r', label="slope: %f    intercept: %f" % (slope, intercept))
ax[0].legend(fontsize=16)
ax[0].grid()

ax[1].tick_params(axis='x', labelsize=16)
ax[1].tick_params(axis='y', labelsize=16)    
ax[1].set_xlabel("$( E^T_{miss,fpga} - E^T_{miss,fw} ) / E^T_{miss,fw}$",loc="right",fontsize=16)
ax[1].set_ylabel("# Events",loc="top",fontsize=16)
ax[1].hist((MET_df["fpga_MET"]-MET_df["fw_MET"])/MET_df["fw_MET"],histtype="step",bins=19,range=(-1,1))
ax[1].grid()
plt.tight_layout()
plt.savefig(name+"metfpgasimvsfpga.png")
